...
$requiredWorkingDays = max(0, (int) ($weeklyGrid['required_working_days'] ?? 0));
$completedWorkingDays = max(0, (int) ($weeklyGrid['completed_working_days'] ?? 0));
$holidayDaysInWeek = max(0, (int) ($weeklyGrid['holiday_days_in_week'] ?? 0));
$missingRequiredDays = max(0, (int) ($weeklyGrid['missing_required_days'] ?? 0));
$isHolidayWeekPendingSubmission = $weekStatus === 'draft' && $holidayDaysInWeek > 0;
$missingDaysMessage = $isHolidayWeekPendingSubmission
    ? sprintf(
        'Semana con %d feriado%s. Completa %d de %d días hábiles para enviar.',
        $holidayDaysInWeek,
...
$resolveActivityApprovalState = static function (string $rawStatus): string {
...
$canSubmitWeek = $hasDraftEntriesInWeek && ($missingRequiredDays <= 0);
$weeklyApprovalTotals = [
    'approved' => 0.0,
    'pending' => 0.0,
    'rejected' => 0.0,
];
foreach ($activitiesByDay as $dayItems) {
    ...
    foreach ($dayItems as $activityItem) {
        if (!is_array($activityItem)) {
            continue;
        }
        $activityState = $resolveActivityApprovalState((string) ($activityItem['status'] ?? ''));
        $weeklyApprovalTotals[$activityState] += (float) ($activityItem['hours'] ?? 0);
    }
}
$activityTypeMeta = [
...
<section class="timesheet-ux">
    ...
    <?php else: ?>
        <header class="timesheet-sticky-header card">
            ...
            <div class="header-actions">
                ...
                <form method="POST" action="<?= $basePath ?>/timesheets/submit-week">
                    ...
                    <button type="submit" class="btn success" <?= !$canSubmitWeek ? 'disabled' : '' ?>>Enviar semana</button>
                </form>
        ...
        <?php endif; ?>
        <?php if ($isHolidayWeekPendingSubmission): ?>
            <section class="card week-notice-banner week-holiday-message"><?= htmlspecialchars($missingDaysMessage) ?></section>
        <?php endif; ?>
        ...
        <section class="card weekly-approval-summary">
            <h3>Resumen semanal de aprobación</h3>
            <div class="approval-summary-grid">
                <article class="approval-summary-item">
                    <span>Horas aprobadas</span>
                    <strong><?= round((float) ($weeklyApprovalTotals['approved'] ?? 0), 2) ?>h</strong>
                </article>
                <article class="approval-summary-item">
                    <span>Horas pendientes</span>
                    <strong><?= round((float) ($weeklyApprovalTotals['pending'] ?? 0), 2) ?>h</strong>
                </article>
                <article class="approval-summary-item">
                    <span>Horas rechazadas</span>
                    <strong><?= round((float) ($weeklyApprovalTotals['rejected'] ?? 0), 2) ?>h</strong>
                </article>
            </div>
        </section>
            ...
            <div class="calendar-column card">
                ...
                <div class="week-calendar-grid">
                    <?php foreach ($gridDays as $day): ?>
                        ...
                        $canEditDay = (!$isBlockedDay || $isHoliday) && !$isWorkflowLocked;
                        ?>
                        <article class="day-card<?= $isHoliday ? ' holiday-day' : '' ?><?= !$isHoliday && $isBlockedDay ? ' non-working-day non-working' : '' ?>" data-drop-day="<?= htmlspecialchars($dayDate) ?>" data-non-working="<?= (!$isHoliday && $isBlockedDay) ? '1' : '0' ?>" data-day-type="<?= htmlspecialchars($dayType) ?>" data-day-name="<?= htmlspecialchars($daySpecialName) ?>" data-day-status="<?= htmlspecialchars($dayStatus) ?>">
                            ...
                            <?php if ($daySpecialName !== ''): ?>
                                <div class="day-special-name<?= $isHoliday ? ' holiday-name' : '' ?>"><?= htmlspecialchars($daySpecialName) ?></div>
                            <?php endif; ?>
                            ...
                            <?php if ($items === []): ?>
                                <div class="day-empty-state">
                                    ...
                                    <?php elseif ($isHoliday): ?>
                                        <small>Este dia es festivo y el registro es opcional.</small>
                            ...
                            <?php else: ?>
                                <ul class="activity-list">
                                    <?php foreach ($items as $item): ?>
                                        ...
                                        $itemApprovalState = $resolveActivityApprovalState((string) ($item['status'] ?? ''));
                                        $itemApprovalMeta = $approvalStateMeta[$itemApprovalState] ?? $approvalStateMeta['pending'];
                                        ...
                                        <li class="activity-chip <?= htmlspecialchars($typeMeta['class']) ?><?= $canEditDay ? ' is-draggable' : ' is-locked' ?>" <?= $canEditDay ? 'draggable="true"' : '' ?> data-activity-id="<?= $itemId ?>" title="<?= htmlspecialchars($chipTooltip) ?>">
                                            ...
                                            <div class="chip-status-row">
                                                <span class="chip-approval-status pill status <?= htmlspecialchars($itemApprovalMeta['class']) ?>">
                                                    <?= htmlspecialchars($itemApprovalMeta['label']) ?>
                                                </span>
                                            </div>
                                            <?php if ($itemApprovalState === 'rejected' && $itemApprovalComment !== ''): ?>
                                                <small class="chip-approval-comment">Comentario aprobador: <?= htmlspecialchars($itemApprovalComment) ?></small>
                                            <?php endif; ?>
                                            <?php if ($canEditDay): ?>
...
.week-holiday-message{border-color:color-mix(in srgb,var(--neutral) 50%,var(--border));background:color-mix(in srgb,var(--neutral) 16%,var(--surface))}
...
.weekly-approval-summary{display:flex;flex-direction:column;gap:10px}
.weekly-approval-summary h3{margin:0}
.approval-summary-grid{display:grid;grid-template-columns:repeat(3,minmax(140px,1fr));gap:10px}
.approval-summary-item{display:flex;flex-direction:column;gap:4px;border:1px solid var(--border);border-radius:12px;padding:10px;background:color-mix(in srgb,var(--surface) 92%,var(--background))}
.approval-summary-item span{font-size:12px;color:var(--text-secondary)}
...
.day-card.holiday-day{background:#f3f4f6;border-color:#d1d5db}
...
.day-special-name{font-size:10px;color:#6b7280;font-weight:600;margin:2px 0 6px}
.day-special-name.holiday-name{font-size:9px;color:#9ca3af;font-weight:600;letter-spacing:.01em}
...
.chip-status-row{display:flex;align-items:center;gap:6px;flex-wrap:wrap}
.chip-approval-status{font-size:11px;padding:3px 8px}
.chip-approval-comment{color:#991b1b;font-size:11px;font-weight:600}
...
@media (max-width: 1100px){.timesheet-sticky-header{grid-template-columns:1fr}.timesheet-main-layout{grid-template-columns:1fr}.day-card{flex:1 1 100%}.quick-add-box{position:static}.indicators-grid{grid-template-columns:repeat(2,minmax(120px,1fr))}.approval-summary-grid{grid-template-columns:1fr}}
</style>
...
(() => {
  ...
  form?.querySelector('[name="date"]')?.addEventListener('change', (event) => {
    ...
    if (meta.blocked && meta.type !== 'holiday') {
  ...
  form?.addEventListener('submit', async (event) => {
    ...
    if (selectedMeta.blocked && selectedMeta.type !== 'holiday') {
    ...
    try {
      ...
      if (duplicateMeta.blocked && duplicateMeta.type !== 'holiday') {
  ...
  document.querySelectorAll('.move-activity').forEach((button) => {
    button.addEventListener('click', async () => {
      ...
      if (moveMeta.blocked && moveMeta.type !== 'holiday') {
  ...
  document.getElementById('duplicate-day-trigger')?.addEventListener('click', async () => {
    ...
    if (sourceMeta.blocked && sourceMeta.type !== 'holiday') {
    ...
    if (duplicateDayMeta.blocked && duplicateDayMeta.type !== 'holiday') {
  ...
  document.querySelectorAll('.register-activity-btn').forEach((button) => {
    button.addEventListener('click', () => {
      ...
      if (targetDate) {
        ...
        if (targetMeta.blocked && targetMeta.type !== 'holiday') {
  ...
  document.querySelectorAll('[data-drop-day]').forEach((dayCard) => {
    dayCard.addEventListener('dragover', (event) => {
      if (weekFullyLocked) return;
      if (dayCard.dataset.nonWorking === '1' && (dayCard.dataset.dayType || '') !== 'holiday') return;
    ...
    dayCard.addEventListener('drop', async (event) => {
      ...
      if (dayCard.dataset.nonWorking === '1') {
        if ((dayCard.dataset.dayType || '') !== 'holiday') {
