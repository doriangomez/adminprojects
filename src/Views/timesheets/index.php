...
<section class="timesheet-ux">
    <div class="timesheet-tabs card">
        ...
        <?php if (!empty($canManageWorkflow) || !empty($canApprove)): ?>
...
.toggle-line{display:flex;flex-direction:row !important;align-items:center;gap:10px !important;padding:4px 0}
.toggle-label{font-size:13px;cursor:pointer;user-select:none}
.switch{position:relative;display:inline-block;width:44px;height:24px;flex-shrink:0}
.switch input{opacity:0;width:0;height:0;position:absolute}
.slider{position:absolute;cursor:pointer;background:#ccc;border-radius:24px;top:0;left:0;right:0;bottom:0;transition:background .2s ease}
.slider:before{content:"";position:absolute;height:18px;width:18px;left:3px;bottom:3px;background:#fff;transition:transform .2s ease;border-radius:50%;box-shadow:0 1px 3px rgba(0,0,0,.15)}
.switch input:checked+.slider{background:#7c3aed}
.switch input:checked+.slider:before{transform:translateX(20px)}
.switch input:focus+.slider{box-shadow:0 0 0 3px rgba(124,58,237,.25)}
